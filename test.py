...
from schema import SCHEMA_AVRO
import os
...
def send_message_to_kafka(data):
    ...
    schema_id = schema_registry_client.register_schema("schema_test_1", Schema(schema_str, 'AVRO'))
    print(schema_id)
    ...
    producer.produce(
        topic="test_topic",
        value=avro_serializer(data, SerializationContext("test_topic", MessageField.VALUE))
    )
    producer.flush()
